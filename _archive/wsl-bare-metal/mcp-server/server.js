#!/usr/bin/env node
/*
 * minimal-mcp-server —— 零依赖的 stdio MCP server
 *
 * 用途：验证 "ChatGPT → OpenAI tunnel → 本机 MCP server" 这条链路。
 *       也是后续自建中继时的最小 MCP server 模板。
 *
 * 定位说明（别误解）：
 *   · 它是"探针 + 模板"，不是 DesktopCommander 的替代品。工具只有 5 个，
 *     能力刻意做窄，目的是让链路问题一眼可辨。
 *   · 它跟 DesktopCommander 一样是**全权限**执行体（能跑任意命令、读写任意路径）。
 *     这是 MCP 文件/终端类 server 的固有属性，靠环境隔离而非进程内护栏兜底。
 *     在 WSL 里跑 = 只能碰 WSL 的文件系统，这本身就是一层隔离。
 *
...
const TOOLS = [
  {
    name: 'run_command',
    description: '在 WSL 里执行一条 shell 命令，返回 stdout/stderr 与退出码。用于查看系统状态、跑构建、调用 CLI。',
    inputSchema: {
      type: 'object',
      properties: {
  ...
  {
    name: 'read_file',
    description: '读取一个文本文件的内容，可按行偏移与行数限制分块读取。',
    inputSchema: {
      type: 'object',
      properties: {
  ...
  {
    name: 'write_file',
    description: '写入文本文件，父目录不存在会自动创建。整文件覆盖。',
    inputSchema: {
      type: 'object',
      properties: {
        path: { type: 'string', description: '文件绝对路径或 ~ 开头路径' },
  ...
  },
  {
    ...
    description: '返回运行本 server 的机器信息：主机名、用户、平台、内核、Node 版本、当前目录。用来确认请求落到了哪台机器上。',
    ...
    case 'get_system_info': {
      return textResult(
        [
          ...
          `homedir  : ${os.homedir()}`,
          `shell    : ${SHELL_BIN}`,
          `is_wsl   : ${fs.existsSync('/proc/sys/fs/binfmt_misc/WSLInterop') || String(process.env.WSL_DISTRO_NAME || '') !== ''}`,
        ].join('\n')
      );
    }
    default:
...
log(`已就绪 | shell=${SHELL_BIN} | max_chars=${MAX_CHARS} | pid=${process.pid}`);
